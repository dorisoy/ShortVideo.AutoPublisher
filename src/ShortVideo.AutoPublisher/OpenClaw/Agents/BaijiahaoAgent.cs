...
/// 百家号代理
/// </summary>
public class BaijiahaoAgent : AgentBase
{
    public override PlatformType Platform => PlatformType.Baijiahao;
    public override string Name => "百家号代理";

    public BaijiahaoAgent(BrowserManager browserManager, AppSettings settings, ILogger<BaijiahaoAgent> logger)
    ...
    protected override async Task<bool> CheckLoginStatusAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            var userElement = await Page.QuerySelectorAsync("[class*='user'], [class*='avatar']");
    ...
    protected override async Task<bool> DoUploadVideoAsync(VideoContent video, IProgress<int>? progress, CancellationToken ct)
    {
        ...
        catch (Exception ex) { Logger.LogError(ex, "百家号视频上传失败"); return false; }
    }
    ...
    protected override async Task<bool> DoSetVideoMetadataAsync(string title, string[] tags, string description, CancellationToken ct)
    {
        ...
        catch (Exception ex) { Logger.LogError(ex, "百家号元数据设置失败"); return false; }
    }
    ...
    protected override async Task<bool> DoSetCoverImageAsync(CoverImage cover, CancellationToken ct)
    {
        ...
        catch (Exception ex) { Logger.LogError(ex, "百家号封面设置失败"); return false; }
    }
    ...
    protected override async Task<PublishResult> DoPublishAsync(CancellationToken ct)
    {
        ...
        catch (Exception ex) { Logger.LogError(ex, "百家号发布失败"); return new PublishResult { Success = false, ErrorMessage = ex.Message }; }
    }
}
